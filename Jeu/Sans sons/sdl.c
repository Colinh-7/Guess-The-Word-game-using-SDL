    ...
    if (pfont && ifont && lfont && title_font) {
        ...
        while (play) {
            ...
            switch(evt.type) {
                ...
                break;
                case SDL_KEYUP:
                    if (evt.key.keysym.sym == SDLK_DOWN) *menu = (++(*menu) == QUIT) ? PLAY : *menu;
                    if (evt.key.keysym.sym == SDLK_UP) *menu = (--(*menu) == MAIN_MENU) ? ADD_LANG: *menu;
                    if (evt.key.keysym.sym == SDLK_RETURN) {
                        if (admin_mode) {
                            play = SDL_FALSE;
                        }
...
char *select_language_window(SDL_Renderer *renderer, SDL_Window *window, char **language_array, int array_size, MenuState *menu)
{
    ...
    //Capitalize all language for better looking on screen
    for (;i<array_size; ++i) {
...
int play_game_window(AVL_Image *image, AVL_Trans *trans, SDL_Renderer *renderer, SDL_Window *window, MenuState *menu)
{
    ...
    if (image && trans && renderer && window) {
        ...
        while (play && ((time_left = difftime(mktime(end_time), time(NULL))) > 0)) {
